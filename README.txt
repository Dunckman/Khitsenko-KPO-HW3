...
**Оговорочка:** почему-то при перепроверке отчёта все отступы и переносы строк в окнах с кодом ниже сбились из-за чего json'ы нечитаемы почти, ну и остальной код в отчёте тоже. Не знаю в чём проблема. Я изначально писал этот редми на сайте StackEdit, потом перенёс в README.md в Rider. Когда перепроверял скопировал из файла и закинул в StackEdit, и отступов уже не было. Зато после исправления в превью самого Rider отображаются огромнейшие отступы. Сейчас, опять скопировав из Rider, ещё и таблицы сломались. Я приложу лучше ещё .txt файл с таким же md кодом. Сори(
